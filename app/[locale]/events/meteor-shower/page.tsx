...
export async function generateMetadata({
    ...
    return {
        ...
        alternates: {
            canonical: `https://theheartopia.com/${locale}/events/meteor-shower`,
            languages: {
                "en": "https://theheartopia.com/en/events/meteor-shower",
                "th": "https://theheartopia.com/th/events/meteor-shower",
                "pt": "https://theheartopia.com/pt/events/meteor-shower",
                "es": "https://theheartopia.com/es/events/meteor-shower",
                "x-default": "https://theheartopia.com/en/events/meteor-shower",
            },
        },
        openGraph: {
            title: t.meteorShower.metaTitle,
            description: t.meteorShower.metaDesc,
            url: `https://theheartopia.com/${locale}/events/meteor-shower`,
    ...
    const breadcrumbSchema = {
        ...
        "itemListElement": [
            ...
            },
            {
                ...
                "position": 3,
                "name": m.heroTitle,
                "item": `https://theheartopia.com/${locale}/events/meteor-shower`
            }
        ]
    }
                ...
                <div className="max-w-4xl mx-auto">
                    ...
                    <header className="mb-12">
                        <div className="flex flex-wrap items-center gap-3 mb-6">
                            <Badge className="bg-orange-500/10 text-orange-600 hover:bg-orange-500/20 border-none px-4 py-1 rounded-full text-sm font-semibold">
                                {m.badge}
                            </Badge>
                            {m.lastUpdated && (
                                <span className="text-sm text-muted-foreground font-medium">{m.lastUpdated}</span>
                            )}
                        </div>
                    ...
                    {m.serverTime && (
                        <section className="mb-16 bg-white rounded-3xl p-8 shadow-sm border border-orange-100">
                            <div className="flex items-center gap-3 mb-6">
                                ...
                                <h2 className="font-serif text-3xl font-bold text-foreground">{m.serverTime.title}</h2>
                            </div>
                            <p className="text-lg text-muted-foreground mb-6">{m.serverTime.desc}</p>
                            <div className="overflow-x-auto">
                                <table className="w-full border-collapse min-w-[320px]">
                                    <thead>
                                        <tr className="bg-heartopia-cream border-b-2 border-orange-100">
                                            <th className="text-left p-4 font-bold text-foreground">{m.serverTime.thTimezone}</th>
                                            <th className="text-left p-4 font-bold text-foreground">{m.serverTime.thStart}</th>
                                            <th className="text-left p-4 font-bold text-foreground">{m.serverTime.thUntil}</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {m.serverTime.rows.map((row: { tz: string; start: string; until: string }, i: number) => (
                                            <tr key={i} className="border-b border-slate-100 hover:bg-orange-50/50">
                                                <td className="p-4 font-medium text-foreground">{row.tz}</td>
                                                <td className="p-4 text-muted-foreground">{row.start}</td>
                                                <td className="p-4 text-muted-foreground">{row.until}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
                    ...
                    <section className="bg-gradient-to-br from-heartopia-cream to-white rounded-3xl p-8 border border-orange-100">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            {m.related.winterBirds && (
                                <Link
                                    href={`/${locale}/guides/winter-birds-location-map`}
                                    className="flex items-center gap-3 p-4 bg-white rounded-xl border border-slate-100 hover:border-heartopia-pink hover:shadow-md transition-all group"
                                >
                                    <ArrowRight className="w-5 h-5 text-heartopia-pink group-hover:translate-x-1 transition-transform" />
                                    <span className="font-medium text-foreground group-hover:text-heartopia-pink-dark transition-colors">
